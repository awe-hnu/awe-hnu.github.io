...
:::fragment
But a network doesn't just physically connect *boxes*, it connects purposeful systems: [information systems]{.link.color}.
:::
...
An Information System (IS) is the **"purposeful system"** that gives meaning to a network connection. It is a structured entity designed for interaction [@krcmar2015informationsmanagement].
...
: Characteristics of an Information System {#tbl-is}

::::

:::fragment
Because no IS is an island, we need a global, standardized mechanism to manage these **interfaces** and **interactions** at scale. [That mechanism is the [Internet]{.link-color}.]{.fragment}
:::

## The Internet

:::fragment
When we take these Information Systems (the nodes) and link them via Computer Networks (the infrastructure) on a global scale, we get the [Internet]{.link-color}.
:::
...
Because the Internet is a [tiered system of private networks]{.highlight} (Tier-1 & Tier-2), the owners of the infrastructure have the technical capability to act as [gatekeepers]{.highlight}.
...
[Information systems]{.medium .fragment} \
[Purposeful systems with interfaces *("No IS is an island")*]{.fragment .fade-in-then-semi-out}

[Networking infrastructure]{.medium .fragment} \
[Tiered ISPs, POPs, and IXPs as the physical backbone]{.fragment .fade-in-then-semi-out}

[Open architecture]{.medium .fragment} \
[BGP, TCP, and IP: universal, layered, and standardized protocols]{.fragment .fade-in-then-semi-out}

[Governance tensions]{.medium .fragment} \
[Open architecture vs. private gatekeepers]{.fragment .fade-in-then-semi-out}
...
[Who really controls an *"open"* network?]{.h4}

- No single controlling authority exists — but the Internet is far from uncontrolled
- Control is *polycentric*, distributed across several layers:
  - **Standards:** IETF (protocols), W3C (web), etc.
  - **Names and numbers:** ICANN and regional registries (DENIC, RIPE, ARIN, ...)
  - **Physical interconnection:** Tier-1 ISPs and IXPs through private peering and transit agreements
  - **National jurisdiction:** governments enforce laws on infrastructure within their borders (e.g., GDPR, the Great Firewall)
  - **De-facto choke points:** Cloudflare, Google, and AWS handle a large share of all traffic
- *Architectural* openness does not imply *political* or *economic* openness

[Is neutrality technically possible, or just a policy goal?]{.h4}

- Technically, discrimination is *trivial:*
  - Deep Packet Inspection (DPI) can identify protocols and applications
  - Quality-of-Service (QoS) mechanisms can prioritize or throttle flows
  - BGP routing policies can favor or penalize specific networks
- *Some* discrimination is even necessary — congestion management, anti-DDoS, low latency for VoIP and real-time applications
- The question is therefore not *"can we be neutral?"* but *"which forms of discrimination are legitimate?"*
- This makes net neutrality fundamentally a *regulatory* and *normative* question (compare the EU Open Internet Regulation with the back-and-forth at the US FCC)

[What happens when *"the pipe"* is also a competitor?]{.h4}

- **Vertical integration** creates structural conflicts of interest:
  - Comcast (ISP) owns NBCUniversal; AT&T previously owned Time Warner
  - Deutsche Telekom and Vodafone bundle their own streaming and video services
- Observable behaviors:
  - **Zero-rating own services** (e.g., StreamOn vs. competing music and video apps)
  - **Throttling competitors** (Comcast vs. Netflix, 2014 — settled with paid peering)
  - **Preferential peering** with affiliated content providers
- This previews the next unit: in distributed architectures (CDNs, cloud), the same logic re-emerges at higher layers — AWS hosts competitors of Amazon retail; Cloudflare sits in front of rival security products
...
:::large
Emerging information technologies enable strongly\
[**interconnected**]{.fragment .fade-in-then-semi-out}
and
[**distributed**]{.fragment .highlight-current-blue}
systems.
:::

[Last week: *connection.*]{.fragment}\
[This week: *distribution.*]{.fragment}

# From connection to distribution {.headline-only}
...
: Comparison of interconnectedness and distributivity {#tbl-diff}
...
:::medium
Is [Mastodon]{.highlight} a good example of the future of social media in Web 3.0?
:::
