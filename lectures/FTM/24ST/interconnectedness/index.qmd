---
title: "Interconnectedness and distributedness"  
...
# Interconnectedness {.headline-only background-color="#0333ff"}
...
[Interdependence]{.link-color}, connotes a stronger relationship in which two systems not only are connected, but depend upon one another in some way, such as functionally. 
...
- TCP guarantees that all bytes are received in the right order by using positive acknowledgements (ACK) with re-transmission^[The receiver responds with an ACK for ever data packet received, sender retransmits packets for missing ACKs after a given time]
::::
...
# Excursus: Net Neutrality {.headline-only background-color="#0333ff"}
...
- Identify key stakeholders in the debate^[Key stakeholder in the net neutrality debate are, for instance, internet service providers (ISPs), content providers, consumers, government, CDNs, etc.]
...
# Distributivity {.headline-only background-color="#0333ff"}
...
Form small groups and take 10 minutes to find examples and identify key differences
...
# Cloud computing {.headline-only background-color="#0333ff"}
...
![The TCP/IP stack and its four abstraction layers](images/cloud-stack-1.svg){.fragment height="420"}
...
:::medium
Research on a hot topic in cloud computing.
:::
...
# The decentralized web {.headline-only background-color="#0333ff"}
