...
-   **Modus Ponens**[^12]: $\frac{\alpha \implies \beta, \ \alpha}{\beta}$\
    ...
    [(e.g., "A implies B" is true, and "A" is true, then "B" must also be true.)]{.fragment}
-   **And-Elimination** (Conjunction Elimination): $\frac{\alpha \land \beta}{\beta}$       
    From a conjunction, any of the conjuncts can be inferred      
    [(e.g., If both "P and Q" are true, then "P" must be true and "Q" must be true)]{.fragment}
:::
...
Complete following truth table.
...
Try to find a real life example for the following equivalences.
