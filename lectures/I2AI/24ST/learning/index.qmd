...
# The learning agent {.vertical-center .headline-only background-color="#564ac6" background-image="../assets/bg.jpeg"}
...
Supervised learning
: Image classification — inputs can be camera images, each one accompanied by an output saying, e.g., "bus" or "pedestrian". An output like this is called a __label__. The agents learns a function that, when given a new image, predicts the appropriate label.
...
# Decision trees {.vertical-center .headline-only background-color="#564ac6" background-image="../assets/bg.jpeg"}
...
How to find a tree that is __consistent with the training set__ (@tbl-res-ts), yet __as small as possible__?

:::{.incremental}
- It is __intractable__ to find a guaranteed smallest consistent tree.
...
# Statistical ML {.vertical-center .headline-only background-color="#564ac6" background-image="../assets/bg.jpeg"}
...
where $P(d|h_i)$ is called the __likelihood__
...
__Maximum a posteriori__ (MAP)[^7] learning: choose $h_{MAP}$ maximizing $P(h_i|d)$[^8].
...
**Maximum likelihood (ML) learning**[^ML] means, thus, to choose $h_{ML}$ maximizing $P(d|h_i)$.
...
➞ Finding MAP hypotheses is often much easier than Bayesian learning[^OR]
...
[^8]: Which is equal to maximizing $P(d|h_i)P(h_i)$ or $\ log P(d|h_i) + \log P(h_i)$
[^OR]: MAP learning provides a natural embodiment of __Ockham's razor__
[^ML]: Maximum likelihood is the “standard” (non-Bayesian) statistical learning method
...
# Deep learning {.vertical-center .headline-only background-color="#564ac6" background-image="../assets/bg.jpeg"}
...
# Development process {.vertical-center .headline-only background-color="#564ac6" background-image="../assets/bg.jpeg"}
...
### Importance {.html-hidden .discussion-slide .unlisted background-color="#efefef"}
...
## Exercise {.html-hidden .discussion-slide .unlisted background-color="#efefef"}
...
: Observations {#tbl-e7-obs}

1. Suppose the robot has a Naive Bayes-based brain. Write the Naive Bayes classification rule for this problem (i.e. write a formula which, given a data point x = (m, b, s), returns the most likely subject y). Write the formula in terms of likelyhood (conditional probabilties) and prior probabilities. 
2. Give estimates for the parameters of the classification rule based on the training samples.
3. Suppose a subject which has stripes but no spots or blue eyes. What would happen if the robot had to guess the identity of the subject? 
