...
:::{.notes}
This introduces the core problem in decision tree learning: we want to generalize from examples, not just memorize them. The smallest consistent tree would likely generalize better, but finding it is computationally infeasible. This is why we need heuristic approaches.
:::
...
:::{.notes}
Pruning is the primary technique for preventing overfitting in decision trees. Pre-pruning is simple but might stop too early. Post-pruning allows the tree to fully grow before simplifying it, which often produces better results but is more computationally expensive. Most implementations provide parameters to control both approaches.
:::
