...
format:
  html:
    ...
    margin-header: |
      ...
      [Solutions](solutions/solutions.html){.btn .btn-secondary target="blank"}
    format-links: false
...
This unit establishes the conceptual framework for the course. All subsequent units cover concrete approaches to implement the [*agent function*](index.qmd#architecture).

| Unit                      | Approach                           | Agent type                          |
|:--------------------------|:-----------------------------------|:------------------------------------|
| **Search & Planning**     | Systematic state-space search      | Goal-based agents                   |
| **Knowledge & Inference** | Logic-based reasoning              | Knowledge-based agents              |
| **Probability Theory**    | Reasoning under uncertainty        | Foundation for utility-based agents |
| **Bayesian Networks**     | Structured probabilistic inference | Utility-based agents                |
| **Introduction to ML**    | Learning from data                 | Learning agents                     |
| **Decision Trees**        | Interpretable classification       | Learning agents                     |
| **Neural Networks**       | Deep representation learning       | Learning agents; Agentic AI         |
...
To analyze systems exhibiting agency, we define the following core components. Note that sensors and actuators are *part of* the agent itself; percept and percept sequence describe what flows in from the environment.

:::incremental
- **Agent:** anything that perceives its environment through sensors and acts upon it through actuators.
- **Sensors:** mechanisms (cameras, GPS, microphones) *within the agent* that receive environmental input.
- **Actuators:** mechanisms (wheels, display screens, robotic joints) *within the agent* that execute actions.
...
:::incremental
- **Agent function:** An abstract mathematical mapping ($f: \mathcal{P}^* \to \mathcal{A}$) that describes how any given percept sequence results in an action, where $\mathcal{P}^*$ denotes the set of all possible percept sequences and $\mathcal{A}$ the set of all possible actions.
...
:::notes
The **performance measure** defines the criteria by which an agent's behavior is evaluated, i.e., what counts as success.
...
To understand the engineering limits of rational agents (and the AI systems that implement them) we distinguish between three standards:
...
| Term                    | Definition                                                                                                                                                                                                                       |
|:------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Agency**              | The capacity of a system to maintain a continuous feedback loop with its environment by mapping percept sequences to actions.                                                                                                    |
| **Agent**               | The conceptual unit of analysis in AI: anything that perceives its environment through sensors and acts upon it through actuators. The term is framework-neutral and applies to systems as simple as a thermostat.               |
| **AI system**           | A concrete computational implementation designed to exhibit intelligent behavior. An AI system typically realizes *one or more agents*, combining hardware, software, and data to perceive, reason, and act in pursuit of goals. |
| **Agentic AI**          | An AI system capable of autonomous, goal-directed behavior over extended tasks — decomposing goals into sub-goals, gathering information, learning from experience, and acting with minimal human intervention.                  |
| **Percept**             | The agent's perceptual input at any given instant.                                                                                                                                                                               |
| **Percept sequence**    | The complete history of everything an agent has ever perceived; the basis for its action choices.                                                                                                                                |
| **Sensors**             | Mechanisms within the agent (e.g., cameras, GPS, microphones) that receive environmental input.                                                                                                                                  |
| **Actuators**           | Mechanisms within the agent (e.g., wheels, display screens, robotic joints) that execute actions.                                                                                                                                |
| **Agent function**      | An abstract mathematical mapping ($f: \mathcal{P}^* \to \mathcal{A}$) from percept sequences to actions.                                                                                                                         |
| **Agent program**       | The concrete implementation of the agent function running on a physical architecture.                                                                                                                                            |
| **Performance measure** | The criteria by which an agent's behavior is evaluated; defines what counts as success.                                                                                                                                          |
| **Rationality**         | The property of an agent that maximizes its *expected* performance given its percept sequence and built-in knowledge.                                                                                                            |
| **Task environment**    | The problem context in which an agent operates, specified via PEAS.                                                                                                                                                              |
| **PEAS**                | A framework for specifying task environments: Performance measure, Environment, Actuators, Sensors.                                                                                                                              |
| **Environment**         | The external context surrounding the agent that provides perceptual inputs and is affected by the agent's actions.                                                                                                               |
| **Autonomy**            | The degree to which an agent relies on its own experience and learning rather than built-in knowledge.                                                                                                                           || **Autonomy**            | The degree to which an agent relies on its own experience and learning rather than built-in knowledge.                                                                                                                           |
