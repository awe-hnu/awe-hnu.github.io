...
format: 
  html:
    ...
    margin-header: | 
      ...
      [Session](session.qmd){.btn .btn-secondary target="blank"} 
    format-links: false        
...
- The agents use **atomic representations**^[An atomic representation is one in which each state is treated as a black box with no internal structure.], where each state is a discrete node in a state space.
- The movement between these states is defined by **actions**, which a **transition model** uses to describe the results of each specific maneuver. 
- The **search algorithms** used to find a path can be either _uninformed_ (blind) or _informed_ (using heuristics).
:::
...
- **Actions:** $\textrm{ACTION}(s)$: a function that returns the set of possible actions available to the agent when in state $s$
- **Transition model:** $\textrm{RESULT}(s, a)$: a function that returns the state $s'$ that results from performing action $a$ in state $s$
- **Goal test:** an abstract condition or a set of goal states[^3] that determine if a state is a destination
- **Action cost function:** $\textrm{ACTION-COST}(s, a, s')$: a function that assigns a numeric cost to performing action $a$ in state $s$ to reach state $s'$
- **Path:** a sequence of actions leading from one state to another.
- **Solution:** a path that leads from the initial state to a goal state. An optimal solution is the solution with the lowest path cost.
:::
...
- **Initial state:** The starting waypoint of the agent (e.g., "Home").
- **Actions:** $\textrm{ACTIONS}(s)$: The set of available road segments or turns (e.g., *Turn Right onto Main St*) at a given intersection.
- **Transition model:** $\textrm{RESULT}(s, a)$: The specific waypoint reached after traveling along a chosen road segment.
- **Goal test:** A condition checking if the current waypoint is the destination (e.g., "Airport Terminal").
- **Action cost function:** $\textrm{ACTION-COST}(s, a, s')$: The distance, time, or fuel consumption for a specific segment.
- **Path:** A sequence of road segments through the network.
- **Solution:** A sequence of road segments leading from the home intersection to the destination.

**Numbered tiles (sliding tiles)**

- **State space:** All possible configurations of the 8 numbered tiles in a $3 \times 3$ grid.
- **Initial state:** Any specific starting arrangement of the tiles and the empty square.
- **Actions:** $\textrm{ACTIONS}(s)$: The possible moves of the blank space: $\{\textit{Left, Right, Up, Down}\}$.
- **Transition model:** $\textrm{RESULT}(s, a)$: The new board configuration after the blank space and an adjacent tile swap positions.
- **Goal test:** Does the current configuration match the target numerical order?
- **Action cost function:** $\textrm{ACTION-COST}(s, a, s')$: A uniform cost of $1$ per move.
- **Path:** A sequence of tile slides.
- **Solution:** The specific sequence of slides that transforms the start state into the goal state.

**8 Queens Puzzle**

- **State space:** Any arrangement of 0 to 8 queens on the board.
- **Initial state:** An empty $8 \times 8$ board.
- **Actions:** $\textrm{ACTIONS}(s)$: Add a queen to any empty square in the next available column.
- **Transition model:** $\textrm{RESULT}(s, a)$: The board configuration with the new queen added.
- **Goal test:** 8 queens are on the board, and none are attacking each other (no two queens in the same row, column, or diagonal).
- **Action cost function:** Zero (the "path" doesn't matter; only the final configuration).
- **Solution:** A final arrangement of 8 non-attacking queens.

Try the *8 Queens Problem* yourself: [https://www.brainbashers.com/queens.asp](https://www.brainbashers.com/queens.asp).

**Chess**

- **State space:** All possible legal board configurations of the 32 chess pieces.
- **Initial state:** The standard setup with all pieces in their starting positions.
- **Actions:** $\textrm{ACTIONS}(s)$: The set of all legal moves for the player whose turn it is.
- **Transition model:** $\textrm{RESULT}(s, a)$: The board configuration resulting from a specific move.
- **Goal test:** Any board configuration where the opponent's king is in checkmate.
- **Action cost function:** $\textrm{ACTION-COST}(s, a, s')$: Typically a uniform cost of $1$ per move.
- **Path:** A sequence of legal moves.
- **Solution:** A sequence of moves leading from the initial position to a checkmate.
...
:::notes
The quality of your problem formulation directly affects:
...
For example, in the *8 Queens Puzzle* problem, a poor modeling choice might represent the entire board as a single entity, while a better model would represent each tile position individually, resulting in a more manageable search tree.
...
# Search algorithms {.headline-only}
...
- Exponential space complexity, i.e., all generated nodes have to be stored until the solution is found.
- **FIFO queue[^FIFO]**
...
- Linear space complexity, i.e., the frontier is very small as subtrees, which have been traversed completely without finding a solution, can be removed, before new subtrees are generated.
- **LIFO queue[^LIFO]**
...
:::notes
The △ nodes are "`MAX` nodes", in which it is `MAX's` turn to move; the ▽ nodes are "`MIN` nodes". 
`MAX's` best move at the root is α₁ (the highest MinMax value achievable at level 3),
`MIN's` best move is β₁ (the lowest minimax value).
:::
...
- This process is repeated up to the root node. `Max` then knows his next action, i.e., the one, which yields to a successor with maximum utility-value.
:::
...
:::incremental
- Alpha-beta pruning maintains two values during the tree traversal:
  - α: the minimum score that the maximizing player (Max) is guaranteed to achieve (worst for Max)
  - β: the maximum score that the minimizing player (Min) is guaranteed to achieve (worst for Min)
- As the algorithm explores the tree:
  - At maximizing nodes ($L-2$), we update alpha if we find a better value
  - At minimizing nodes ($L-1$), we update beta if we find a better value
If at any node $k$ α ≥ β, we can stop exploring the current node's remaining children (this is the pruning)
:::
...
# Monte Carlo tree search  {.headline-only}
