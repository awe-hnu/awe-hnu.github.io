...
:::html-hidden
In the [neural networks unit](../neural-networks/index.qmd) we built a language model from the ground up:
:::
...
- **Next-token prediction:** the [transformer](../neural-networks/index.qmd#transformers) learns language by predicting what comes next, then generates text one token at a time
...
:::fragment
This shift from recognition to generation is fundamental [@goodfellow2016deep; @urbach2026genai]:
:::
...
:::html-hidden
:::medium
An LLM is the [transformer](../neural-networks/index.qmd#transformers) you already built, scaled up and trained on internet-scale text [@vaswani2017attention; @brown2020language].
:::
:::
...
:::incremental
- **Same mechanism:** tokenize, embed, attend, predict the next token (see [neural networks unit](../neural-networks/index.qmd#transformers))
...
The rest of this section focuses on what is genuinely new relative to the neural networks unit: how raw next-token prediction is turned into a helpful assistant through a three-phase training process, and where LLMs are applied in practice [@sanderson2024gpt].
:::

## Training phases

The next-token prediction you saw last unit is only the **first of three training phases** [@ouyang2022training]:

:::incremental
1. **Pretraining:** self-supervised [next-token prediction](../neural-networks/index.qmd#training) on massive, unlabelled text corpora (internet archives, books, scientific articles); the model learns linguistic patterns, factual associations, and reasoning structures, exactly the process from the neural networks unit
...
:::medium
The neural networks unit introduced the core failure modes; deploying LLMs at scale raises the stakes and adds new ones [@riemer2023; @verma2023].
:::

:::fragment
[Carried over from the transformer]{.h4}
:::
