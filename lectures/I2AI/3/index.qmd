...
- The child node to be considered next needs to be selected using some __evaluation function__ `f(n)` (__strategies__ see below)
...
:::{.notes}
- __Breadth-first search:__ expands the shallowest nodes first   
  - complete, optimal for unit action costs
  - exponential space complexity
  - FIFO queue
  - a reached table is necessary to store the nodes already reached
- __Depth-first search:__ expands the deepest unexpanded node first  
...
- __Uniform-cost search:__ expands the node with the lowest path costs
...
- __Priority queue__ first pops the node with the minimum costs according to some evaluation function (uniform-cost search)
- __FIFO queue__ or first-in-first-out queue first pops the node that was added to the queue first (breadth-first search)
- __LIFO queue__ or last-in-first-out queue (also known as a stack) pops first the most recently added node (depth-first search)
:::

:::{.small}

#### Process of the breadth-first search algorithm
...
#### Process of the uniform-cost search algorithm
...
![Result for I2AI_3 question b](images/maze.svg){#fig-maze}
