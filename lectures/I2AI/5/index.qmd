...
Although propositional logic suffices to represent the wumpus world, it is quite complicated

- Rules must be set up for __each square__
- We need a __time inde__ for each proposition that refer to an aspect of the world that changes to represent the validity of the proposition over time, which causes further expansion of the rules

More powerful logics exist, in which we can use object variables, e.g. First-Order Predicate Logic
