...
:::{.incremental}
- Moves for both players according the playout policy[^5] are chosen
- The moves are not recorded in the search tree
- In @fig-mcts-expansion, the simulation results in a win for `P-B`
:::
:::

[^5]: Playout policies biases the moves toward good ones. For Go and other games, playout policies have been successfully learned from self-play by using neural networks. Sometimes also game-specific heuristics are used (e.g., take the corner square in Othello)
