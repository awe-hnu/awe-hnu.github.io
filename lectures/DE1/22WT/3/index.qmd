...
-   verstehen Sie das Konzept des **Cloud-Computings** inkl. der wichtigsten Bereitstellungs- und Service-Modelle und
...
-   **Ausgabe:**_ Zur Präsentation oder Weiterleitung von Bearbeitungsergebnissen stehen verschiedene Geräte zur Verfügung.
...
Rechnernetze (oder kurz Netzwerke) dienen der **Übertragung von Daten** zwischen mehreren Rechnern indem Sie Rechner über Übertragungswege miteinander verbinden. Dadurch ermöglichen Netzwerke unterschiedliche Formen zur computergestützten Interaktion, Kommunikation und Transaktion [@leimeister2021einfuhrung].
...
Das **Datenübertragungsverfahren** definiert, wie die Daten zwischen Daten Datenstationen über den Übertragungsweg ausgetauscht werden. Die Verfahren werden von den Protokollen und Architekturen festgelegt (wie bspw. die TCP/IP Protokollfamilie). Siehe hierzu auch das [OSI-Referenzmodell](https://de.wikipedia.org/wiki/OSI-Modell).
:::
...
-   **Platform as a Service (PaaS)**: Die Bereitstellung von Entwicklungsumgebungen, in der Regel mit integrierter Datenbank sowie Programmier- und Laufzeitumgebungen und Werkzeugen zum Management der Anwendungen. Mit Hilfe dieser vorkonfigurierten Diensten können Nutzer ihren eigenen Anwendungen entwickeln, ohne die zugrundeliegende Infrastruktur verwalten zu müssen. Die Plattformen sind in der Regel auf verschiedene Industrien oder Anwendungsdomänen zugeschnitten.
...
System- und Anwendungssoftware sind oft **Open Source Software**.
...
Welche Open Source Software kennen Sie?

Recherchieren Sie zur [GNU General Public License V3](https://opensource.org/licenses/GPL-3.0), die beispielsweise für Linux verwendet wird. Erläutern Sie was man mit Open Source Software unter dieser Lizenz (nicht) machen darf.

## Cloud Servicemodelle

Recherchieren Sie auf der Website von [Microsoft Azure](https://azure.microsoft.com) und/oder [Amazon Web Services](https://aws.amazon.com/) je **Servicemodell** (IaaS, PaaS, SaaS) min. ein **konkretes Angebot**.

Beschreiben Sie das Angebot in Ihren eigenen Worten.

Überlegen Sie, wie Unternehmen diese Services für die Entwicklung von Produkten, die menschlich erwünscht, technologisch machbar und wirtschaftlich tragfähig sind. Geben sie möglichst konkrete Beispiele.
...
-   Infrastruktur und (betriebliche) Anwendungen werden heute oft als Services aus der Cloud bezogen. Nennen Sie eines der in der Vorlesung besprochenen Service-Modelle und erläutern Sie dieses jeweils anhand eines konkreten Beispiels.
