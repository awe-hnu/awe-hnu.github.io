---
title: "Leader"
subtitle: "What makes an effective leader?"
lang: en

bibliography: ../assets/literature.bib

title-slide-attributes:
  data-background-image: "../assets/bg.jpg"
  data-background-size: cover
  data-background-opacity: "1"
  data-background-color: '#0333ff'

date: 03.30.2026

format:
  html:
    output-file: index.html
    margin-header: |
      [Slides](slides.html){.btn .btn-primary target="blank"}
    format-links: false
  presentation-revealjs:
    output-file: slides.html
    include-before-body: ../assets/footer.html
---

:::{.content-visible when-format="revealjs"}
# Agenda

:::medium
- Warm-up [15 min]{.smaller}
- Evolution of leadership [25 min]{.smaller}
- Big Five & paradoxes [35 min]{.smaller}
- [Break]{.highlight}
- LTEE model [20 min]{.smaller}
- Emotional intelligence [20 min]{.smaller}
- Wrap-up [10 min]{.smaller}
:::

:::{.content-visible when-format="revealjs"}
:::notes
135 minutes total.

The session is designed as a flipped classroom: students have pre-read @judge2002personality and answered three guiding questions.
:::
:::
:::

# Learning Objectives

After completing this unit, you will be able to:

::: incremental
1. Trace the evolution of leadership thought from Great Man theory to complexity leadership.
2. Analyze how the Big Five personality traits relate to leadership emergence and effectiveness.
3. Explain the LTEE model and the interplay of genetics, traits, and context in leader emergence.
4. Assess both the bright and dark sides of personality traits in leadership contexts.
5. Reflect on your own trait profile and emotional intelligence as a foundation for leadership development.
:::

:::{.content-visible when-format="revealjs"}
# Warm-up {.headline-only}

## Latticework Check-In {.discussion-slide}

:::large
What insights from [Unit 1](../foundations/index.qmd) might help us think about leader traits?
:::

:::notes

Land on the map vs. territory connection: trait models are useful simplifications, not the full picture.

> Traits are a map of personality, not the full person. Any trait model simplifies a rich, complex human being into measurable dimensions. Useful, but incomplete.

Alternatives: circle of competence, second-order thinking, VUCA.
:::

## Reflection {.discussion-slide}

:::large
Write down your findings.
:::

1. **One key finding** from @judge2002personality that **confirmed** something you expected.
2. **One finding** that **surprised** you.

:::fragment
:::large
Turn to your neighbor\
and compare your findings.
:::

Where do you agree? Where did your surprises differ?
:::

{{< countdown "4:00" top="0">}}

:::

# Evolution of Leadership {.headline-only}

## A Brief History

Before we examine what specific traits matter, it helps to understand how our *thinking about leadership* has evolved. 

:::fragment
Each era emphasized different aspects and each provided useful *mental models.*
:::

:::{.content-visible when-format="revealjs"}
:::fragment
:::medium
Great Man → Trait → Behavioral → Contingency → Transformational → Complexity
:::
:::
:::

## Great Man Theory (1840s)

:::medium
Great leaders **are born,** not made.
:::

:::fragment
@carlyle1841heroes argued that history is shaped by exceptional individuals with innate heroic qualities. Leadership was seen as a gift bestowed on a select few (typically male, aristocratic, and military).
:::

:::{.content-visible unless-format="revealjs"}
The Great Man Theory reflects the 19th-century belief that leadership capacity is inherent and cannot be developed. While largely discredited, it planted the seed for trait-based research: if great leaders are "different," what exactly makes them different? This question drove decades of subsequent research.
:::

:::{.content-visible when-format="revealjs"}
:::notes
The Great Man Theory reflects the 19th-century belief that leadership capacity is inherent and cannot be developed. While largely discredited, it planted the seed for trait-based research: if great leaders are "different," what exactly makes them different? This question drove decades of subsequent research.
:::
:::

## Trait Approaches (1930s–1940s)

:::medium
Leadership depends on the **personal qualities** of the leader.
:::

:::fragment
Early researchers sought universal traits that distinguish leaders from non-leaders.

Initial reviews [e.g., @stogdill1948personal] found inconsistent results, leading many to abandon the trait approach until meta-analyses revived it decades later (e.g., @judge2002personality).
:::

:::{.content-visible unless-format="revealjs"}
The trait approach represents a more scientific evolution of the Great Man idea. Rather than assuming leaders are "born great," researchers tried to *measure* what traits correlate with leadership. The approach fell out of favor mid-century because no single trait seemed universally predictive. It was later revived with more sophisticated methods, notably the Big Five framework and meta-analyses like @judge2002personality.
:::

:::{.content-visible when-format="revealjs"}
:::notes
The trait approach represents a more scientific evolution of the Great Man idea. 

Rather than assuming leaders are "born great," researchers tried to *measure* what traits correlate with leadership. 

The approach fell out of favor mid-century because no single trait seemed universally predictive. 
It was later revived with more sophisticated methods, notably the Big Five framework and the meta-analysis the students read.
:::
:::

## Behavioral Theories (1950s–1960s)

:::medium
If traits alone don't explain leadership, perhaps **what leaders do** matters more than who they are.
:::

:::fragment
Two important research programs:

- **Ohio State Studies** identified *initiating structure* (task-oriented) and *consideration* (relationship-oriented) as key leadership behaviors [@Fleishman1953].
- **Michigan Studies** distinguished *production-oriented* and *employee-oriented* leadership [@Katz1950].
:::

:::{.content-visible unless-format="revealjs"}
The behavioral approach shifted the focus from traits to observable actions. Its key insight: leadership can be *learned* through developing the right behaviors, not just born. This opened the door to leadership development programs. However, the approach struggled to account for the fact that the same behaviors work well in some situations but not others, which led to contingency theories.
:::

:::{.content-visible when-format="revealjs"}
:::notes
The behavioral approach shifted the focus from traits to observable actions. 

Its key insight: **leadership can be *learned* through developing the right behaviors.** 

- **Ohio State Studies** identified two independent dimensions of leader behavior. 
  - Initiating structure is about organizing work: defining roles, setting goals, establishing procedures, and pushing for task completion. 
  - Consideration is about building trust and rapport: showing concern for followers' well-being, being approachable, and fostering a supportive climate. 
  - The key insight was that these are independent: a leader can be high on both, low on both, or any combination.
- **Michigan Studies** arrived at a similar distinction from a different angle. 
  - Production-oriented leaders emphasize efficiency, technical aspects of the job, and getting results. 
  - Employee-oriented leaders emphasize interpersonal relationships, take interest in followers' needs, and accept individual differences. 
  - Unlike Ohio State, the Michigan researchers initially framed these as **opposite ends of a single continuum (either/or),** though later work softened that position.

This opened the door to leadership development programs. 

However, the approach struggled to account for the fact that the same behaviors work well in some situations but not others, which led to contingency theories.
:::
:::

## Contingency Theories (1960s–1970s)

:::medium
There is no single best way to lead.\
**Effectiveness depends on the situation.**
:::

:::fragment
Key models:

- **Fiedler's Contingency Model** [@fiedler1967theory] states that leader effectiveness depends on the match between leadership style and situational favorability.
- **Hersey & Blanchard's Situational Leadership** [@hersey1977management] advises leaders to adapt their style (i.e., telling, selling, participating, delegating) based on follower readiness.
:::

:::{.content-visible unless-format="revealjs"}
Contingency theories represent a major conceptual leap: they reject the idea of universally effective leadership and instead argue that context matters. @fiedler1967theory proposed that leaders have relatively fixed styles and should be matched to appropriate situations. @hersey1977management took a more flexible view, arguing leaders can and should adapt. We will revisit situational and adaptive leadership in depth in Unit 3.
:::

:::{.content-visible when-format="revealjs"}
:::notes
Contingency theories reject the idea of universally effective leadership and argue that context matters. 

Fiedler proposed that leaders have relatively fixed styles and should be matched to appropriate situations. 

- **Telling** (high directive, low supportive): The leader provides specific instructions and closely supervises. Used when followers are unable and lack confidence; they need clear guidance on what, how, and when.
- **Selling** (high directive, high supportive): The leader still defines tasks but explains decisions and encourages dialogue. Used when followers are willing but not yet able; they need direction but also motivation and buy-in.
- **Participating** (low directive, high supportive): The leader shares decision-making and facilitates collaboration. Used when followers are able but unwilling or insecure; they have the skills but need encouragement and involvement to commit.
- **Delegating** (low directive, low supportive): The leader hands over responsibility for decisions and execution. Used when followers are both able and willing; they can be trusted to run with the task independently.
- The core idea is that there's no single best style: effective leadership means diagnosing follower readiness (a combination of ability and willingness) and matching the style accordingly. As followers develop, the leader should shift from telling toward delegating.

Hersey and Blanchard took a more flexible view, arguing leaders can and should adapt. 

We will revisit situational and adaptive leadership in depth in Unit 3.
:::
:::

## Transformational Leadership (1980s–1990s)

:::medium
Leaders transform followers **by inspiring them** to transcend self-interest for the sake of the organization.
:::

:::incremental
- @burns1978leadership distinguished **transactional leadership** (exchange-based) from **transformational leadership** (purpose-based).
- @bass1985leadership operationalized **transformational leadership:** idealized influence, inspirational motivation, intellectual stimulation, individualized consideration.
:::

:::{.content-visible unless-format="revealjs"}
Transformational leadership theory brought emotion, vision, and meaning back into the leadership conversation. It explained why some leaders inspire extraordinary commitment while others, equally competent, do not. The theory has strong empirical support and remains one of the most widely studied leadership frameworks. Its emphasis on *vision* and *inspiration* connects directly to our later unit on storytelling (Unit 9).
:::

:::{.content-visible when-format="revealjs"}
:::notes
Transformational leadership brought emotion, vision, and meaning back into the conversation. It explained why some leaders inspire extraordinary commitment while others, equally competent, do not. Strong empirical support. Connects directly to storytelling in Unit 9.
:::
:::

## Complexity Theories (2000s+)

:::medium
In complex adaptive systems, leadership is **distributed,** **emergent,** and **contextual.**
:::

:::fragment
- @uhl2007complexity introduced **Complexity Leadership Theory** conceptualizing leadership as enabling adaptability in complex systems
- @heifetz1994leadership introduced **Adaptive Leadership** distinguishing technical problems (known solutions) from adaptive challenges (require learning and changed behavior)
:::

:::{.content-visible unless-format="revealjs"}
Complexity and adaptive leadership theories reflect the realities of the VUCA world we discussed in [Unit 1](../foundations/index.qmd#the-decision-making-crisis). They argue that in highly complex, interconnected environments, the "heroic leader" model is insufficient. Instead, leadership must be understood as a system-level phenomenon that emerges through interactions. We will explore the adaptive leadership framework [@heifetz1994leadership] in depth in Unit 3.
:::

:::{.content-visible when-format="revealjs"}
:::notes
These theories reflect the VUCA world from Unit 1. The "heroic leader" model is insufficient in highly complex environments. Leadership must be understood as a system-level phenomenon that emerges through interactions. We explore adaptive leadership in depth in Unit 3.
:::
:::

:::{.content-visible when-format="revealjs"}
## Paradigm Pitch {.discussion-slide}

:::medium
Prepare a **60-second pitch** for one paradigm.
:::

1. Why is your paradigm's mental model **still useful today?**
2. Give a **concrete example** from the digital era.

{{< countdown "5:00" top="0">}}

:::{.content-visible when-format="revealjs"}
:::notes
**Group task (10 min, groups of 3–4).**

- Assign paradigms by table or draw randomly (prepare cards labeled: Trait, Behavioral, Contingency, Transformational, Complexity; skip Great Man).
- Give exactly 5 minutes for preparation, then 60-second pitches.
- After each pitch, add one sentence of commentary connecting it to the broader arc.
- End by showing the synthesis table on the next slide.
:::
:::
:::

## Overview

Each paradigm offers a **mental model** for understanding leadership, and each remains partially useful:

| Era              | Core mental model                | Still useful for...       |
|------------------|----------------------------------|---------------------------|
| Traits           | Individual differences matter    | Self-awareness, selection |
| Behavioral       | Actions can be learned           | Leadership development    |
| Contingency      | Context determines effectiveness | Situational judgment      |
| Transformational | Vision inspires commitment       | Organizational change     |
| Complexity       | Systems produce emergence        | Digital transformation    |

:::{.content-visible unless-format="revealjs"}
No single paradigm "won." Each captured something the others missed. This is precisely the latticework idea from [Unit 1](../foundations/index.qmd#latticework-thinking): effective leadership thinking requires multiple models. In this unit, we focus on traits, with the understanding that it is one map among many.
:::

:::{.content-visible when-format="revealjs"}
:::notes
**Plenary synthesis (5 min).** Show this table as the capstone. Emphasize: no single paradigm "won." This is precisely the latticework idea: effective leadership thinking requires multiple models. In this unit, we focus on traits, with the understanding that it is one map among many. Transition to Block 3: "So let's look more closely at the trait map you explored in your pre-reading."
:::
:::

# The Big Five {.headline-only}

:::{.content-visible when-format="revealjs"}

## Height Bias {.discussion-slide .no-headline .vertical-center}

![Dilbert comic strip on leadership](images/dilbert.jpg){#fig-dilbert fig-align="left"}

## Discussion {.discussion-slide}

:::large
What traits **distinguish leaders**\
from other people?
:::

:::{.content-visible when-format="revealjs"}
:::notes
**Plenary discussion (5 min).**

- Open the floor and collect answers on the board.
- Then reveal the regression figure on the next slide.
- Probe the findings students found surprising (reference their index cards from Block 1).
- Push on these questions:
  - Why is extraversion dominant for emergence but less so for effectiveness?
  - Why is agreeableness nearly irrelevant? Is that a problem?
  - What does this mean for introverted leaders?
:::
:::
:::

:::{.content-visible unless-format="revealjs"}

## Big Five Traits

The Big Five traits have been found to be relevant to many aspects of life, such as subjective well-being [e.g., @deneve1998happy] and even longevity [@friedman1995childhood]. They also provide a valuable taxonomy for the study of leadership [@judge2002personality].

Openness to Experience
: This trait reflects **a person's willingness to explore new ideas, be curious, and engage with novel experiences.** Those high in openness tend to be creative, imaginative, and interested in art, music, and diverse cultural experiences. They're often intellectually curious and comfortable with abstract thinking. Those lower in openness may prefer familiarity, concrete thinking, and conventional approaches.

Conscientiousness
: This trait describes **a person's tendency to be organized, disciplined, and achievement-oriented.** Highly conscientious individuals are typically reliable, thorough, and methodical in their approach to tasks. They value planning, order, and completing responsibilities. Those lower in conscientiousness may be more spontaneous, disorganized, and might prioritize flexibility over structure.

Extraversion
: This trait reflects **a person's preference for social interaction and external stimulation.** Extraverts tend to be energetic, talkative, and gain energy from being around others. They often seek excitement and are comfortable in group settings. Introverts (those low in extraversion) typically prefer quieter environments, deeper one-on-one interactions, and need time alone to recharge.

Agreeableness
: This trait describes **a person's tendency toward cooperation and social harmony.** Highly agreeable individuals are typically compassionate, considerate, and eager to help others. They value getting along with people and avoiding conflict. Those lower in agreeableness tend to be more competitive, skeptical, and willing to challenge others' views.

Neuroticism (sometimes referred to as Emotional Stability)
: This trait reflects **a person's tendency to experience negative emotions and psychological distress.** Individuals high in neuroticism may experience more anxiety, mood swings, irritability, and emotional reactivity. Those low in neuroticism (high in emotional stability) tend to be more calm, emotionally resilient, and less easily upset by stressors.
:::

## Big Five x Leadership

![Regression of Leadership on Big Five Traits according to the meta study of @judge2002personality](images/leadershipBigFive.svg){#fig-bigFive fig-align="left"}

:::{.content-visible unless-format="revealjs"}
**Leadership effectiveness** refers to a leader's performance in influencing and guiding the activities of his or her unit toward achievement of its goals [@stogdill1950leadership].

**Leadership emergence** refers to whether (or to what degree) an individual is viewed as a leader by others, who typically have only limited information about that individual's performance [@hogan1994we].

Leadership effectiveness and emergence represent two levels of analysis. Leadership emergence is a within-group phenomenon. In contrast, leadership effectiveness represents a between-groups phenomenon.

Regarding these two constructs and personality traits, the meta-analysis of @judge2002personality shows that:

- The Big Five typology is a fruitful basis for examining the dispositional predictors of leadership.
- **Extraversion** is the most important trait of leaders and effective leadership.
- Extraversion is more strongly related to leader emergence than to leader effectiveness.
- **Extraversion,** **Conscientiousness** and **Openness to Experience** were the strongest and most consistent correlates of leadership.
- Conscientiousness is more strongly related to leader emergence than to leadership effectiveness.
- **Agreeableness** was the least relevant trait.
- **Neuroticism** failed to emerge as a significant predictor of leadership.

Overall, the traits are better predictors in situations in which individuals have only limited opportunity to observe leadership behaviors (i.e., student samples). However, the results may simply indicate a close correspondence between the way we see people's personalities and our stereotypical conceptions of the characteristics of leaders.
:::

:::{.content-visible when-format="revealjs"}
:::notes
Key findings to highlight:

- **Extraversion** is the most important trait for leadership overall, but more strongly related to emergence than effectiveness.
- **Conscientiousness** and **Openness to Experience** were the strongest and most consistent correlates alongside extraversion.
- **Agreeableness** was the least relevant trait.
- **Neuroticism** failed to emerge as a significant predictor.
- Traits are better predictors when observers have limited opportunity to observe behavior (student samples). This may reflect stereotypical conceptions of leaders rather than actual effectiveness.

Probe: "Does this mean we should select leaders based on extraversion?" Push students toward nuance.
:::
:::

:::{.content-visible when-format="revealjs"}
## Trait Paradoxes {.discussion-slide}

:::medium
Analyze a **Big Five trait**
:::

Work through these questions:

1. Summarize the **bright side** and **dark side** of your trait.
2. Create a **concrete leadership scenario** where the trait helps and one where it **backfires.**
3. Is this trait **more or less important in the digital era?** Why?

{{< countdown "10:00" top="0">}}

:::notes
**Group work**

- Assign one trait per group (Extraversion, Agreeableness, Conscientiousness, Emotional Stability, Openness).
- Encourage groups to think beyond the table and create vivid, specific scenarios (e.g., "An extraverted leader dominates a Zoom call and misses quiet dissent from remote team members").
- Walk around and push groups that finish early to deepen their digital-era argument.
:::
:::

## Trait Paradoxes

| Trait               | Bright Side                                                                                           | Dark Side                                                                                          |
|---------------|-----------------------------|----------------------------|
| Extraversion        | [Greater leadership emergence; higher job and life satisfaction]{.fragment}                          | [More impulsive (deviant) behaviors; more accidents]{.fragment}                                    |
| Agreeableness       | [Higher subjective well-being; lower interpersonal conflict; lower turnover]{.fragment} | [Lower career success; less capable of conflict; more lenient in giving ratings]{.fragment} |
| Conscientiousness   | [Stronger job performance; higher leadership effectiveness; lower deviance]{.fragment}               | [Reduced adaptability; lower learning in initial stages of skill acquisition]{.fragment}           |
| Emotional stability | [High job/life satisfaction; better job performance; effective leadership; retention]{.fragment}     | [Poorer ability to detect risks and danger; more risky behaviors]{.fragment}                       |
| Openness            | [Higher creativity; greater leadership effectiveness; greater adaptability]{.fragment}               | [More accidents and counterproductive; rebelliousness; lower commitment]{.fragment}                |

: Trait paradoxes as identified by @judge2009bright (more paradoxes in the paper) {#tbl-paradoxes}

:::{.content-visible when-format="revealjs"}
## Break {.headline-only}
:::

## Evolution

:::{.content-visible when-format="revealjs"}

We know which traits matter. But where do they come from?

:::large
Leaders are born.
:::

:::fragment
Really?
:::

:::fragment
Traits have a **genetic source** [and are the result of **adaptive processes** [[@judge2009bright].]{.smaller}]{.fragment .fade-in}
:::

:::fragment
The statement, thus, **oversimplifies a complex phenomenon** that involves an interplay between genetic factors, developmental experiences, learning, and situational contexts.
:::

:::

:::{.content-visible unless-format="revealjs"}
Studies of identical twins separated at birth have indeed provided valuable insights into the genetic basis of personality traits, including those associated with leadership. Twin studies typically show heritability estimates of 30-60% for leadership tendencies, suggesting a significant genetic component.
However, the research doesn't support a purely deterministic "born leader" perspective for several reasons:

1. Even with identical twins raised apart, the heritability of leadership traits is never 100%. Environmental factors still play a substantial role.
2. Leadership comprises multiple traits and abilities (like extraversion, assertiveness, decision-making skill, emotional intelligence) that may each have different genetic influences.
3. The expression of genetic predispositions often depends on environmental triggers and opportunities (gene-environment interaction).
4. Leadership development research shows that many leadership skills can be learned and improved through experience, mentoring, and deliberate practice.
5. Cultural and situational factors heavily influence who emerges as a leader and what leadership styles are effective.

A more nuanced view supported by contemporary research is that certain individuals may have genetic predispositions that make leadership roles more natural or appealing to them, but these predispositions require the right environmental conditions, learning experiences, and opportunities to develop into effective leadership.

> "Asking how much a particular individual's attitudes or traits are due to heredity versus the environment is nonsensical, just like asking whether a leaky basement is caused more by the crack in the foundation or the water outside." @olson2001heritability [*pp. 845-846*]
:::

:::{.content-visible when-format="revealjs"}
:::notes
**Mini-lecture start (8 min total).**

Acknowledge the grain of truth: twin studies show 30–60% heritability for leadership tendencies.
But heritability is never 100%, and expression depends on environment.
Use the quote:

> Asking how much a particular individual's attitudes or traits are due to heredity versus the environment is nonsensical, just like asking whether a leaky basement is caused more by the crack in the foundation or the water outside *@olson2001heritability [pp. 845–846]*
:::
:::

## The LTEE Model

::: {.r-stack .html-hidden}
![The Leader Trait Emergence Effectiveness (LTEE) model [@judge2009bright]](images/process-1.svg){.fragment height="420"}

![&nbsp;](images/process-2.svg){.fragment height="420"}

![&nbsp;](images/process-3.svg){.fragment height="420"}

![&nbsp;](images/process-4.svg){.fragment height="420"}

![&nbsp;](images/process.svg){.fragment height="420"}

:::


:::{.content-visible unless-format="revealjs"}

![The Leader Trait Emergence Effectiveness (LTEE) model [@judge2009bright]](images/process.svg){#fig-process fig-align="left"}

The Leader Trait Emergence Effectiveness (LTEE) model combines the behavioral genetics and evolutionary psychology theories of how personality traits are developed into a model that explains leader emergence and effectiveness [@judge2009bright].

- The **genetics** are the foundational biological basis for leadership potential. Genetic predispositions influence personality traits, cognitive abilities, emotional regulation as well as neurological characteristics that support leadership behaviors.
- **Selection processes** refer to the mechanisms by which potential leaders are identified and developed. While genes determine the expression of traits, evolutionary
processes, in organizations and in collectives more generally, determine which traits are "selected in" and "selected out."
- **Traits** are individual characteristics that contribute to leadership potential.
- **Mediators** are intervening factors that translate traits into leadership behaviors. Based on Socioanalytic Theory @judge2009bright argue that the reason traits result in leader emergence is because of the motives the traits elicit. For example,
  - conscientious, extraverted, and emotionally stable (or high core self-evaluations) individuals may be more motivated to get ahead (agency, competing)
  - extraverted and agreeable individuals may be more likely to go along (communion, cooperation),
  - and intelligent, and charismatic individuals may be more likely to provide meaning for their compatriots
- **Leader emergence** is the process of transitioning from potential to recognized leadership, which depends on the context and the adaptive processes. As adaptive and coordination problems increase, so does the importance of leadership. There is little need for prudent, courageous and flexible leadership when the collective faces little conflict from within or without. Thus, leader emergence occurs through recognition and opportunity.
- **Subjective effectiveness** reflects perceived leadership quality (e.g., peer receptions, follower satisfaction, reputation and social standing).
- **Objective effectiveness** demonstrates tangible leadership impact (e.g., team productivity, goal achievement, financial performance).
:::

:::{.content-visible when-format="revealjs"}
:::notes

1. **Genetics:** foundational biological basis. Genetic predispositions influence personality, cognition, emotional regulation.
2. **Selection processes:** evolutionary and organizational mechanisms that select certain traits in or out.
3. **Traits:** individual characteristics. The Big Five we just discussed.
4. **Mediators:** the bridge between traits and emergence. Based on Socioanalytic Theory: conscientious/extraverted/stable individuals are motivated to *get ahead* (agency); extraverted/agreeable individuals *go along* (communion); intelligent/charismatic individuals *provide meaning*.
5. **Emergence to Effectiveness:** emergence is context-dependent (as adaptive challenges increase, leadership becomes more important). Effectiveness splits into subjective (reputation, follower satisfaction) and objective (productivity, goal achievement).

6. Genetics set a range; mediators and context determine where in that range you land. This is where development happens.
:::
:::

:::{.content-visible when-format="revealjs"}
## Think-Pair-Share {.discussion-slide}

:::medium
If traits have a genetic source,\
can leadership really be **developed?**
:::

Where in the LTEE model do you see **room for development?**

:::small
2 minutes individual thinking\
3 minutes pair discussion.
:::

{{< countdown "5:00" top="0">}}

:::{.content-visible when-format="revealjs"}
:::notes
**Think-Pair-Share (7 min).** Individual thinking (2 min), pair discussion (3 min), collect 3–4 responses in plenary (2 min). Expected answers: mediators (motives and skills can be trained), context (you can seek the right situations), selection processes (organizations can design better pipelines). If students focus only on traits, redirect: "The model shows traits are just one input; what about the arrows between boxes?"

**Plenary synthesis (5 min).** Land the key insight: the LTEE model shows that trait-based determinism is too simple. Development happens at the mediator level, and one of the most researched mediator frameworks is emotional intelligence. Bridge: "Goleman argues that EI is precisely what can be learned. Let's explore that."
:::
:::
:::

# Emotional Intelligence

Intelligence is the most "successful" trait in social and applied psychology [@judge2009bright].

:::fragment
@goleman1998makes argues that **emotional intelligence,** not IQ (intelligence, a trait), sets apart great leaders.
:::

:::fragment
Emotional intelligence is a group of **five skills** that enable the best leaders to maximize their own and their followers' performance:
:::

::: incremental
-   **Self-awareness:** knowing one's strengths, weaknesses, drives, values, and impact on others.
-   **Self-regulation:** controlling or redirecting disruptive impulses and moods.
-   **Motivation:** relishing achievements for its own sake.
-   **Empathy:** understanding other people's emotional makeup and treating them accordingly.
-   **Social skill:** building rapport with others to move them in desired directions.
:::

:::{.content-visible when-format="revealjs"}
:::notes
Brief framing (2 min) before the jigsaw.

- Connect EI to the LTEE model: emotional intelligence operates at the mediator level, translating traits into effective leadership behaviors.
- Goleman's central claim is that EI can be learned, which makes it the actionable lever in the model.
- Introduce the five skills very briefly, then move directly into the jigsaw.
:::
:::

:::{.content-visible when-format="revealjs"}
## EQ Skills Exercise {.discussion-slide}

Do this on your own (7 minutes):

:::incremental
1. **Research** the core characteristics of your assigned skill\
   (@goleman1998makes can be found on Moodle).
2. **Find or construct** a specific leadership example.
3. Identify **one pathway** to strengthen this skill.
4. Is this skill **gaining or losing importance** in digital/hybrid work? Why?
:::

:::fragment
Then group into **mixed teams** (one expert per skill)\
and **teach your skill to the group** in 2 minutes (10 minutes).
:::

:::fragment
:::medium
Which EI skill is the **hardest to develop?**
:::
:::

{{< countdown "18:00" top="0">}}

:::notes

Expected patterns:
- students often identify self-regulation as hardest (requires sustained effort against ingrained patterns) and empathy as most impactful for digital contexts (harder to read people through screens).
- Push back if consensus forms too quickly: "Is empathy really harder digitally, or do we just need different signals?"

Land on Goleman's foundational argument:

> Self-awareness is the prerequisite for all other EI skills. A leader who doesn't understand their own emotional patterns cannot regulate them, cannot authentically motivate, cannot accurately read others.
>
This connects to the cognitive biases discussion from Unit 1: our self-image is another map that may not match the territory.
:::
:::

## Self-Awareness

Self-awareness, the first component of emotional intelligence, is arguably the **foundation of all leadership development.**

:::fragment
:::medium
Leaders who understand their own strengths, weaknesses, values, and emotional patterns can better manage themselves, relate to others, and adapt to different situations.
:::
:::

:::{.content-visible unless-format="revealjs"}
Research consistently shows that self-aware leaders are more effective. They make sounder decisions because they understand their own biases. They build stronger relationships because they can accurately read how their behavior affects others. And they develop faster because they know where to focus their growth efforts.

Self-awareness connects directly to what we discussed about mental models in [Unit 1](../foundations/index.qmd#mental-models): just as the "map is not the territory," our self-image is not a perfect representation of who we actually are.

Cognitive biases, particularly confirmation bias in self-perception, can distort our self-understanding. Seeking disconfirming evidence about ourselves is just as important as seeking it in our decision-making.
:::

:::{.content-visible when-format="revealjs"}
:::notes
Connect self-awareness to the full arc of the session:
- we started with traits (the map),
- saw that they have paradoxes (the territory is more complex),
- learned that development happens through mediators like EI,
- and now arrive at the starting point of all development: knowing yourself.
:::
:::

:::{.content-visible when-format="revealjs"}
## Personal Reflection {.discussion-slide}

Based on what you have learned about the Big Five and emotional intelligence:

:::incremental
1. **Where do you see your strengths?**\
   Which traits and EI skills come naturally to you?
2. **Where are your potential dark sides?**\
   Every strength has a shadow. Where might your strengths become liabilities?
3. **What is your development edge?**\
   Which EI skill, if strengthened, would have the greatest impact on your leadership?
:::

{{< countdown "5:00" top="0">}}

:::{.content-visible when-format="revealjs"}
:::notes
**Individual reflection (5 min).**

This is private writing, not shared with the group.
Emphasize that this is not about labeling yourself but about building the habit of reflective self-assessment that characterizes effective leaders.
Suggest the IPIP Big Five short form as an optional follow-up tool for students who want empirical data to supplement their self-reflection.

Remember: as @goleman1998makes notes, emotional intelligence *can be learned*. The first step is honest self-assessment and the willingness to act on what you find.
:::
:::
:::

# Conclusion

:::large
It is fortunate, then, that emotional intelligence can be learned.\
[@goleman1998makes]{.smaller}
:::

:::{.content-visible unless-format="revealjs"}
Learning might mean here being able to deal efficiently with our personality traits that have a genetic source.
:::

:::{.content-visible when-format="revealjs"}

# Latticework Update {.discussion-slide}

:::large
Which new models have you added to your latticework?
:::

:::fragment
- Nature–nurture interaction as a thinking framework
- Trait paradoxes: every strength has a shadow
- Cognitive biases in self-perception and other-perception
:::

{{< countdown "5:00" top="0">}}

:::notes
**Plenary closing (5 min).**

*How do these new models connect to the ones we already had?*

Research consistently shows that self-aware leaders are more effective. They make sounder decisions because they understand their own biases.

Expected connection: map vs. territory applies to self-perception (our self-image is a map); second-order thinking applies to trait paradoxes (the second-order effect of a strength is often a weakness).

Preview the homework: @lavine2014paradoxical on paradoxical leadership and the CVF. Connect it explicitly to today's trait paradoxes: "If every trait has a bright and dark side, how do leaders manage both simultaneously? That's what Lavine calls behavioral complexity."
:::
:::

# Homework

Read @lavine2014paradoxical and answer following questions:

- What is ambidexterity, what is behavioral complexity?
- Why does a complex world require behavioral complexity?
- Which leadership paradoxes are identified by the CVF?\
  Can you give specific examples for each?
- How do the paradoxes relate to the specifics of the digital era?

# Q&A {.html-hidden .unlisted .headline-only}

# Literature

::: {#refs}
:::
